...
pub enum CiSubcommand {
    ...
    Once {
        ...
        #[arg(
            long,
            help = "clone/fetch 深度（最小为 1，未设置时=当前 PR commit 数）"
        )]
...
pub enum ReviewSubcommand {
    ...
    Once {
        ...
        #[arg(
            long,
            help = "追加到 diff_file 顶部的自定义提示词（建议 shell 模式使用）"
        )]
        engine_prompt: Option<String>,
        #[arg(
            long,
            help = "提示词文件路径，读取文件内容并追加到 diff_file 顶部（建议用于多行提示词）"
        )]
        engine_prompt_file: Option<String>,
        #[arg(
            long,
            help = "统一提示词模板；可在 --engine 命令里用 {prompt_template} 或 {prompt} 引用；模板内支持 {diff_file}/{agents_file}/{repo_dir}/{repo_head_sha}/{repo_head_ref} 与 PF 语法"
        )]
