...
pub enum ReviewSubcommand {
    ...
    Once {
        ...
        #[arg(
            long,
            help = "统一提示词模板；可在 --engine 命令里用 {prompt_template} 或 {prompt} 引用；模板内支持 {diff_file}/{agents_file}/{repo_dir}/{repo_head_sha}/{repo_head_ref}"
        )]
